...
- Conversation compaction is rolling and token-aware. The effective token trigger is `min(CONVERSATION_SUMMARY_TRIGGER_TOKENS, 250 + 25% of usable input)`; the count trigger is `CONVERSATION_SUMMARY_TRIGGER_MESSAGES`. After either is crossed, the server preserves the latest complete turn plus up to six recent messages within a roughly `T/4` target, folds the older tail, returns `conversation_folded_message_count`, and the browser advances `conversation_compacted_through`. The single rolling summary lives on the conversation, not each message, and is hard-capped at `min(768, max(256, T/8))`. Prompt packing separately prioritizes the minimum chunks, summary, latest turn, all additional chunks except the final one, older raw turns, then the final chunk; lack of prompt space alone does not trigger compaction.
