...
- Default backend `msearch`, hybrid. It can return very short keyword snippets — watch grounding quality.
- `top_k = 0` is not "few sources" but retrieval off entirely; the UI offers it on purpose.
- Optional cross-encoder reranking (`RERANKER_*`, default `BAAI/bge-reranker-v2-m3`) runs after first-stage retrieval. It loads with `local_files_only=True`, so the model must be pre-downloaded; otherwise the rerank controls simply stay hidden.
...
- Sources panel: retrieval order while streaming, then a flip to first-citation order when the answer finishes — but never for aborted/errored streams, retrieve-only mode, or an answer that cites nothing. The view state is a parameter of `renderSourceCards`, shared by the main, conversation and history panels: do not turn it back into a module global. The rest of the ordering, labelling and cited/uncited rules are pinned by `frontend/sources-panel.test.js`.
- Conversation compaction is rolling and token-aware: crossing either the token or the message trigger folds the older tail, the server returns `conversation_folded_message_count`, and the browser advances `conversation_compacted_through`. The single rolling summary lives on the conversation, not on each message. Compaction and prompt packing are separate decisions — lack of prompt space alone must not trigger compaction. The triggers, caps and packing order are pinned by `tests/test_conversation_compaction.py` and `tests/test_token_budget.py`.
- Reasoning: declared per model in `data/models.json` (request field, effort vocabulary, default, `mandatory`, human-only `note`), and discovered from the provider's own catalogue where one publishes it — only OpenRouter's `/api/v1/models` does, so only OpenRouter is asked (`LLM_PROVIDER_<ID>_DISCOVER_CATALOG` overrides). A hand-written entry beats discovery, because it is where a probe of what the model actually did gets recorded. Nothing is sent for a model neither declares. An empty/absent `efforts` list with `mandatory: true` is the "reasons anyway, cannot be steered" case: no control is offered, no parameter is sent, the trace is still shown collapsed. `mandatory` and a `none`/`off` effort contradict each other and the loader strips the switch — gpt-oss-120b shipped both and 400ed on every answer. Add models to the JSON, never to code.
- The effort sent when nobody touches the selector is derived, not declared: `mandatory` → the cheapest level the model accepts, optional → nothing at all (`ReasoningSupport.effective_default`). Lowering a mandatory model changes only how much it thinks; sending a level to an optional one changes whether it thinks, which would silently alter answers for users who never asked. Declare `default` only to overrule that, as the four steerable mandatory GPT-OSS routes now do with `medium`. `mistral-medium-3.5` is why the derivation is not a blanket "low": it accepts only `none`/`high`.
- Context windows come from two places. e-infra publishes `model_info.context_size` at `/model/info`; OpenRouter publishes `context_length` in the same `/models` records as its reasoning metadata, so both are read in one request. A discovered length is clamped to the provider's `context_window` in `data/models.json` (raise `max_context_window` there to lift it) — it is a capacity, not a budget, and a 1M-token window reaching the browser as the working window turns trimming off. The clamp means discovery can only ever say a model is *smaller* than the provider default assumed, which is the case that was over-packing.
...
- An upstream error body must be read *inside* the `with client.stream(...)` block (`_raise_for_status_with_body`). Once the block exits the response is closed and the body is gone, so reading it later reported httpx's "without having called `read()`" for every 4xx/5xx — a real 429 looked identical to a bad model name. `tests/test_llm_errors.py` pins it, using a one-shot iterator body because a `content=`-backed mock stays rewindable after close and would pass either way.
...
- Base prompt helpers live in `app/rag/prompts.py`; WP-specific built-in prompts live in `app/rag/wp_config.py`. Read the prompts there before changing behaviour — among other things they tell the model to answer in the question's language, to cite only sources it used, and not to write its own closing source list or `[^Zn]:` footnote definitions.
